...
public class DepositMenu extends Menu {
    ...
    public DepositMenu(Console console, Atm atm) {
        super(console, atm);
    }
    ...
    public void handleMenu() throws NoSuchDenominationException {
        ...
        if (!AVAILABLE_DENOMINATIONS.contains(selected)) {
            ConsoleHandler.handle(console, ConsoleConstants.WELCOME_MENU, ConsoleConstants.DEFAULT_INSTRUCTION);
        } else {
            Integer notesAmount = ConsoleParser.parse(console, ConsoleConstants.EMPTY_STRING, ConsoleConstants.DEPOSIT_NOTES_INSTRUCTION);
            ConsoleHandler.printBalance(TransactionReceiver.accept(new Deposit(notesAmount * selected, atm)), console, atm);
        }
    }
}
