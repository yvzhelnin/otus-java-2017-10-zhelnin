...
public class BigNotesCounter extends NotesCounter {
    ...
    public List<Note> count() throws NoSuchDenominationException {
        ...
        final int bigNotesAmount = amountRequested / DenominationClassifier.BIG_DENOMINATION;
        for (int i = 0; i < bigNotesAmount; i++) {
            notes.add(new Note(Denomination.ONE_THOUSAND));
        }
        int otherNotesAmount = amountRequested - bigNotesAmount * DenominationClassifier.BIG_DENOMINATION;
